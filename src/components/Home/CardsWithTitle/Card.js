...
const Card = ({ Icon, name, desc, count }) => {
  const [animatedCount, ref] = useCountUp(count);

  return (
    <div
      ref={ref}
      className="py-4 sm:py-6 lg:py-10 xl:py-12 px-5 sm:px-6 lg:px-9 xl:px-10 title-color bg-white flex flex-col gap-y-5 rounded-4xl hover:bg-gradient-to-b hover:from-[#29a7b1] hover:to-[#3bc2cb] hover:bg-clip-border hover:shadow-2xl transition duration-300 ease-in border border-gray-200 group"
    >
      <span>{Icon}</span>
